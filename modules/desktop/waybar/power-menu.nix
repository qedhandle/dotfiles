  ...
  narbix.waybar.provides.power-menu = {
    ...
    homeManager = { config, lib, ... }:
      let
        shutdown = {
          ...
          value = {
            format = mkIcon "F011";
            on-click = config.desktop.shutdown;
            tooltip = true;
        ...
        reboot = {
          ...
          value = {
            format = mkIcon "F021";
            on-click = config.desktop.reboot;
            tooltip = true;
        ...
        sleep = {
          ...
          value = {
            format = mkIcon "F186";
            on-click = config.desktop.sleep;
            tooltip = true;
            tooltip-format = "Sleep";
          };
        };
        lock = lib.optional (config.desktop.lock != null) {
          name = "lock";
          value = {
            format = mkIcon "F023";
            on-click = config.desktop.lock;
            tooltip = true;
            tooltip-format = "Lock";
          };
        };
      ...
      in
    {
      programs.waybar = {
        ...
        //
        lib.listToAttrs
          (builtins.map (item: item // { name = "custom/power-${item.name}"; }) items);
      };
    };
  };
}
